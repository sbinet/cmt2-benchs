...
#---------------------------------------------------------
function(cmt_show_project_uses)
  message("cmt_show_project uses>")
  ...
  foreach(project IN LISTS projects)
    ...
    get_property(uses GLOBAL PROPERTY CMT_${project}_USES)
    message ("Project ${project} in ${path} uses=${uses}")
  endforeach()
endfunction()
...
function(cmt_run command)
  ...
  execute_process(COMMAND ${command} OUTPUT_VARIABLE out)
  message("out=[${out}]")
...
#---------------------------------------------------------
function(cmt_configure_project project name)
  plustab()
...
function(cmt_use_project name)
  ...
  get_property(uses_projects GLOBAL PROPERTY CMT_USED_PROJECT)
  list(FIND uses_projects ${name} i)
  if(i EQUAL -1)
    cmt_message("Configuring the new project ${name}")
    # register this new project
    cmt_message("cmt_append_unique_property(CMT_USED_PROJECT ${name})")
    cmt_append_unique_property(CMT_USED_PROJECT "${name}")
    cmt_message("cmt_append_unique_property(CMT_${project}_USES ${name})")
    cmt_append_unique_property(CMT_${project}_USES "${name}")
  else()
    cmt_message("Project ${name} has been already configured")
    cmt_configure_project(${project} ${name})
  endif()
...
function(cmt_action)
  ...
  if("${action}" STREQUAL "show project uses")
    cmt_show_project_uses()
